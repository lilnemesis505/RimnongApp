...
import 'package:rimnongapp/service/permission_service.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  final String _promptPayId = '0984873750';
  final String _bankName = 'ธนาคารกรุงไทย';
  final String _accountName = 'นายพงศกร มณีสาย';

  Future<void> _pickSlipImage() async {
    final permissionGranted = await PermissionService.requestPhotosPermission();
    if (!mounted || !permissionGranted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ไม่ได้รับอนุญาตให้เข้าถึงรูปภาพ')),
      );
      return;
    }
  ...
  String get _promptPayPayload {
    ...
    String payload = '00020101021229370016A0000006770101110113${_promptPayId.padLeft(13, '0')}5802TH54${amount.toStringAsFixed(2).length.toString().padLeft(2, '0')}${amount.toStringAsFixed(2)}5303764';
      ...
      if (response.statusCode == 201 && data['status'] == 'success') {
        ...
        Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
            ...
            settings: RouteSettings(arguments: widget.orderData['cus_id']), // Pass cusId back
          ),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(data['message'] ?? 'เกิดข้อผิดพลาดในการสั่งซื้อ'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('การเชื่อมต่อผิดพลาด: $e'), backgroundColor: Colors.red),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('หน้าชำระเงิน', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Sarabun')),
        centerTitle: true,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text('สแกนเพื่อชำระเงินด้วย PromptPay', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.brown, fontFamily: 'Sarabun'), textAlign: TextAlign.center),
            ...
            Center(
              child: Container(
                ...
                child: QrImageView(
                  ...
                  eyeStyle: const QrEyeStyle(eyeShape: QrEyeShape.square, color: Colors.brown),
                  dataModuleStyle: const QrDataModuleStyle(dataModuleShape: QrDataModuleShape.square, color: Colors.brown),
                ),
              ),
            ),
            const SizedBox(height: 24),
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('ยอดที่ต้องชำระ', style: TextStyle(fontSize: 18, color: Colors.brown, fontFamily: 'Sarabun')),
                    const SizedBox(height: 8),
                    Text('฿${totalPrice.toStringAsFixed(2)}', style: const TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: Colors.brown, fontFamily: 'Sarabun')),
            ...
            const Text('อัปโหลดสลิปโอนเงิน', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.brown, fontFamily: 'Sarabun'), textAlign: TextAlign.center),
            const SizedBox(height: 16),
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(16),
                  image: _slipImage != null ? DecorationImage(image: FileImage(_slipImage!), fit: BoxFit.cover) : null,
                ),
                child: _slipImage == null
                    ? Center(
                        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                          ...
                          const SizedBox(height: 8),
                          Text('แตะเพื่อเลือกสลิป', style: TextStyle(fontSize: 16, color: Colors.brown[400], fontFamily: 'Sarabun')),
                        ]),
                      )
                    : null,
              ),
  ...
  Widget _buildPaymentDetailRow(String label, String value) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label, style: TextStyle(fontSize: 16, color: Colors.grey[600], fontFamily: 'Sarabun')),
          Text(value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.brown, fontFamily: 'Sarabun')),
        ],
      ),
    );
  }
